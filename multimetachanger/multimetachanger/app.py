...
def render_caption_overlay(text, font_size, font_name, video_w, video_h):
    """
...
def modify_video_metadata(input_path, output_path, reencode=False, custom_meta=None, captions_enabled=False, font_size=48, font_name="arial", caption_text=""):
    """
    ...
    if reencode:
        ...
        if captions_enabled and caption_text:
            ...
            result = render_caption_overlay(caption_text, font_size, font_name, video_w, video_h)
            if result:
                tmp_png, cap_h = result
                # Position Y : centrée verticalement (légèrement au-dessus du centre)
                y_pos = (video_h - cap_h) // 2
                cmd = [
                    ffmpeg_path,
                    "-i", input_path,
                    "-i", tmp_png,
                    "-map_metadata", "-1",
                    "-filter_complex",
                    f"[0:v][1:v]overlay=(W-w)/2:{y_pos}[v]",
                    "-map", "[v]",
                    "-map", "0:a?",
                ]
            # fallback drawtext si Pillow absent
            else:
                fontfile_path = f"C\\:/Windows/Fonts/{font_name}.ttf"
                drawtext_filter = (
                    f"drawtext=fontfile='{fontfile_path}':"
                    f"text='{caption_text}':"
    ...
    try:
        ...
        return False, f"FFmpeg error: {e.stderr[:300]}"
    except Exception as e:
        return False, str(e)
    finally:
